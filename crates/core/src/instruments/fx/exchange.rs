...
impl Exchange {
  ...
  pub fn add_rate(&mut self, rate: ExchangeRate) {
    let key =
      format!("{}/{}", rate.from_currency.code.alphabetic, rate.to_currency.code.alphabetic);
  ...
  pub fn get_rate(
    ...
    let key = format!("{}/{}", from_currency.code.alphabetic, to_currency.code.alphabetic);
    self.rates.get(&key)
  }
  ...
  pub fn convert(&self, money: Money, to_currency: Currency) -> Money {
    let rate = self.get_rate(&money.currency, &to_currency).unwrap_or_else(|| {
      panic!(
...
impl ExchangeRate {
  ...
  pub fn convert(&self, money: Money) -> Money {
    if money.currency == self.from_currency {
      ...
      panic!("The currency of the money doesn't match with from_currency of the exchange rate.")
    }
  }
}
