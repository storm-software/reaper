...
impl Book {
  ...
  #[must_use]
  pub fn new() -> Self {
    Self { buy_limits: BTreeMap::new(), sell_limits: BTreeMap::new(), order_map: HashMap::new() }
  }
  ...
  pub fn add_order(
    ...
    let limit_tree = if is_buy { &mut self.buy_limits } else { &mut self.sell_limits };
  ...
  pub fn cancel_order(&mut self, order_id: u64) -> Result<(), NonExistingIdError> {
    match self.order_map.remove(&order_id) {
      Some(o) => {
        let limit_tree = if o.is_buy { &mut self.buy_limits } else { &mut self.sell_limits };
  ...
  pub fn execute_market_order(&mut self, shares: u64, is_buy: bool) -> (bool, Vec<(u64, u64)>) {
    ...
    let limit_tree = if is_buy { &mut self.sell_limits } else { &mut self.buy_limits };

    while shares_left > 0 {
      let limit_key_value =
        if is_buy { limit_tree.values_mut().next() } else { limit_tree.values_mut().last() };
