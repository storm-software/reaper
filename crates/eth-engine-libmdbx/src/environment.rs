...
impl Environment {
  ...
  pub fn begin_rw_txn(&self) -> Result<Transaction<RW>> {
    ...
    let txn = loop {
      ...
      if matches!(&res, Err(Error::Busy)) {
        if !warned {
          warned = true;
          warn!(target: "reaper-eth-engine-libmdbx", "Process stalled, awaiting read-write transaction lock.");
        }
  ...
  ///
  /// ```
  /// # use reaper_eth_engine_libmdbx::Environment;
...
impl EnvironmentBuilder {
  ...
  pub fn open_with_permissions(&self, path: &Path, mode: ffi::mdbx_mode_t) -> Result<Environment> {
    ...
    #[cfg(feature = "read-tx-timeouts")]
    let txn_manager = {
      if let crate::MaxReadTransactionDuration::Set(duration) =
        self.max_read_transaction_duration.unwrap_or(
          read_transactions::MaxReadTransactionDuration::Set(DEFAULT_MAX_READ_TRANSACTION_DURATION),
        )
      {
