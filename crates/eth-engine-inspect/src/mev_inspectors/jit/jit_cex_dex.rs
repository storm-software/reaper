...
impl<DB: LibmdbxReader> JitCexDex<'_, DB> {
  fn inspect_block_inner(
    ...
    jit_bundles
      .into_iter()
      .filter_map(|jits| {
        tracing::trace!("Checking if classified JITs are actually JIT Cex Dex- {:#?}", jits);
        ...
        if self.cex_dex.is_triangular_arb(&dex_swaps) {
          ...
          self
            ...
            .utils
            .get_metrics()
            .inspect(|m| m.branch_filtering_trigger(MevType::JitCexDex, "is_triangular_arb"));
        ...
        self
          .cex_dex
          .gas_accounting(&mut possible_cex_dex, &tx_info.gas_details, metadata.clone());

        let (profit_usd, cex_dex, trade_prices) =
          self
            .cex_dex
            .filter_possible_cex_dex(possible_cex_dex, &tx_info, metadata.clone())?;
