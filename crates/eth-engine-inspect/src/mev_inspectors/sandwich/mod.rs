...
impl<DB: LibmdbxReader> SandwichInspector<'_, DB> {
    ...
    fn calculate_sandwich(
        ...
        if !(possible_front_runs_info
            ...
            || possible_front_runs_info
                ...
                == 1)
        {
            tracing::debug!(target: "reaper_eth_engine_inspect::sandwich", "all sandwiches don't have same eoa and aren't all verified contracts");
            return None
        }
    ...
    fn has_pool_overlap(
            ...
            victim_actions.iter().zip(victim_info).enumerate()
        {
            ...
            if front_run_pools.intersection(&back_run_pools).count() == 0 {
                tracing::trace!(target: "reaper_eth_engine_inspect::sandwich", "no pool intersection for frontrun / backrun");
            }
    ...
    fn verify_sandwich_victims(
        ...
        trace!(
            target: "reaper_eth_engine_inspect::sandwich",
            "\nGrouped victims: {:#?}\n\
             Front-run tokens: {:#?}\n\
             Back-run tokens: {:#?}\n\
             Front-run pools: {:#?}\n\
        ...
        if amount == 0 {
            trace!(target: "reaper_eth_engine_inspect::sandwich", "no grouped victims");
            return false
        }
...
mod tests {
    ...
    use reaper_eth_engine_types::constants::{DAI_ADDRESS, USDT_ADDRESS, WETH_ADDRESS};
