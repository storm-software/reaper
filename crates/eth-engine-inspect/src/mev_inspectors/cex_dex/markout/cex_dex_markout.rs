...
use reaper_eth_engine_database::libmdbx::LibmdbxReader;
use reaper_eth_engine_metrics::inspectors::OutlierMetrics;
use reaper_eth_engine_types::{
  ...
  BlockData, FastHashMap, MultiBlockData, ToFloatNearest, TreeCollector, TreeSearchBuilder, TxInfo,
};
...
impl<DB: LibmdbxReader> CexDexMarkoutInspector<'_, DB> {
  ...
  fn should_filter_tx(&self, tx_info: &TxInfo) -> bool {
    if let Some(contract_type) = tx_info.contract_type.as_ref() {
      if contract_type.is_defi_automation() {
        trace!(
            target: "reaper-eth-engine::cex-dex-markout",
        ...
        self
          .utils
          .get_metrics()
          .inspect(|m| m.branch_filtering_trigger(MevType::CexDexTrades, "is_defi_automation"));
        return true
      }
    }
    false
  ...
  fn process_dex_swaps(
    ...
    if dex_swaps.is_empty() {
      if let Some(extra) =
        self
          .utils
          .cex_try_convert_transfer_to_swap(transfers, &tx_info, MevType::CexDexTrades)
      {
    ...
    if self.is_triangular_arb(&dex_swaps) {
      ...
      self
        .utils
        .get_metrics()
        .inspect(|m| m.branch_filtering_trigger(MevType::CexDexTrades, "is_triangular_arb"));
      return None
    }
  ...
  fn process_swaps(
    ...
    let header: reaper_eth_engine_types::mev::BundleHeader = self.utils.build_bundle_header(
      vec![deltas],
      vec![tx_info.tx_hash],
      &tx_info,
      profit_usd,
  ...
  pub fn filter_possible_cex_dex(
    ...
    let is_cex_dex_bot_with_significant_activity =
      info.is_searcher_of_type_with_count_threshold(MevType::CexDexTrades, FILTER_THRESHOLD * 2);
    ...
    let tx_attributes_meet_cex_dex_criteria = !info.is_classified
      && info.is_private
      && (info.is_searcher_of_type_with_count_threshold(MevType::CexDexTrades, FILTER_THRESHOLD)
        || info
          .contract_type
          .as_ref()
      ...
      || is_outlier_but_not_stable_swaps
    {
      ...
      self
        .utils
        .get_metrics()
        .inspect(|m| m.branch_filtering_trigger(MevType::CexDexTrades, "filter_possible_cex_dex"));
      None
    }
  }
...
mod tests {
  ...
  use reaper_eth_engine_types::constants::USDT_ADDRESS;
  ...
  async fn test_cex_dex_markout_perl() {
    ...
    let config = InspectorTxRunConfig::new(Inspectors::CexDexMarkout).with_mev_tx_hashes(vec![tx]);
