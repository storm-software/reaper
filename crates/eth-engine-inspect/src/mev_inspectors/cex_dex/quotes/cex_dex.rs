      ...
      .filter_map(|(tx, swaps)| {
        ...
        if dex_swaps.is_empty() {
          if let Some(extra) =
            self
              .utils
              .cex_try_convert_transfer_to_swap(transfers, &tx_info, MevType::CexDexQuotes)
          {
        ...
        if dex_swaps.is_empty() {
          trace!(    target: "reaper-eth-engine::cex-dex-quotes",
        ...
        if self.is_triangular_arb(&dex_swaps) {
          ...
          self
            .utils
            .get_metrics()
            .inspect(|m| m.branch_filtering_trigger(MevType::CexDexQuotes, "is_triangular_arb"));
  ...
  fn filter_possible_cex_dex(
    ...
    if is_cex_dex_based_on_historical_activity || should_include_based_on_pnl {
      let t2 = self
        ...
        .into_iter()
        .map(|quote_option| quote_option.map_or(0.0, |quote| quote.maker_taker_mid().0.to_float()))
        .collect_vec();
...
mod tests {
  ...
  use reaper_eth_engine_types::constants::{USDT_ADDRESS, WBTC_ADDRESS, WETH_ADDRESS};
