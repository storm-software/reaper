...
pub fn get_usdc_address() -> Address {
  convert::to_address("0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48")
}
...
pub fn calculate_next_block_base_fee(block: Block<TxHash>) -> eyre::Result<U256> {
  ...
  let base_fee = block.base_fee_per_gas.ok_or(eyre::eyre!("Block missing base fee per gas"))?;
  ...
  target_gas_used = if target_gas_used == U256::zero() { U256::one() } else { target_gas_used };
...
pub fn read_env_vars() -> Result<Vec<(String, String)>> {
  ...
  let keys =
    vec!["RPC_URL", "RPC_URL_WSS", "PRIVATE_KEY", "FLASHBOTS_AUTH_KEY", "SANDWICH_CONTRACT"];
  for key in keys {
...
pub async fn get_ws_provider() -> Result<Provider<Ws>> {
  ...
  Provider::<Ws>::connect(&url).await.map_err(|e| eyre::eyre!("RPC Connection Error: {:?}", e))
}
