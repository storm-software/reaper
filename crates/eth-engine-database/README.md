...
- [Reaper - Ethereum Engine Database Crate](#reaper---ethereum-engine-database-crate)
...
# Reaper - Ethereum Engine Database Crate
...
```toml
[dependencies]
reaper-eth-engine-database = "0.0.1"
```

Or, via the command line:

```bash
cargo add reaper-eth-engine-database
```
...
Run `nx build reaper-eth-engine-database` to build the library.

### Running unit tests

Run `nx test reaper-eth-engine-database` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint reaper-eth-engine-database` to run [ESLint](https://eslint.org/) on
the package.
