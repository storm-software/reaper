...
use reaper_eth_engine_types::{
  ...
  unordered_buffer_map::ReaperEthEngineStreamExt,
...
impl<TP: TracingProvider, CH: ClickhouseHandle> LibmdbxInitializer<TP, CH> {
    ...
    D: LibmdbxData<T> + DbRow + for<'de> Deserialize<'de> + Send + Sync + Debug + Unpin + 'static,
  {
    ...
    let range_chunks = if let Some((s, e)) = range {
      ...
      (DEFAULT_START_BLOCK..=end_block)
        .chunks(T::INIT_CHUNK_SIZE.unwrap_or((end_block - DEFAULT_START_BLOCK + 1) as usize))
    };

    let pair_ranges = range_chunks
      .into_iter()
      .map(|chk| chk.into_iter().collect_vec())
      .filter_map(|chk| if !chk.is_empty() { Some((chk[0], chk[chk.len() - 1])) } else { None })
      .collect_vec();
  ...
  async fn load_builder_config_data(&self) {
    ...
    let builder_config_str =
      std::fs::read_to_string(builder_config_path).expect("Failed to read builder config file");
  ...
  async fn load_searcher_config_data(&self) {
    ...
    let searcher_config_str =
      std::fs::read_to_string(searcher_config_path).expect("Failed to read searcher config file");
  ...
  async fn load_address_metadata_config(&self) {
    ...
    let config_str = std::fs::read_to_string(workspace_dir).expect("Failed to read config file");
...
mod tests {
  ...
  use itertools::Itertools;
  use reaper_eth_engine_core::test_utils::{get_db_handle, init_trace_parser};
  use reaper_eth_engine_database::libmdbx::{
    initialize::LibmdbxInitializer, tables::*, test_utils::load_clickhouse,
  };
  use reaper_eth_engine_types::init_thread_pools;
