...
pub const PARTITION_FILE_NAME: &str = "reaper-eth-engine-db-partition";
pub const FULL_RANGE_NAME: &str = "reaper-eth-engine-complete-range";
...
impl LibmdbxPartitioner {
  ...
  pub fn execute(self) -> eyre::Result<()> {
    ...
    while start_block + DEFAULT_PARTITION_SIZE < end_block {
      ranges.push(BlockRangeList { start_block, end_block: start_block + DEFAULT_PARTITION_SIZE });
