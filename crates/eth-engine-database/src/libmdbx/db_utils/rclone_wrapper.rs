...
impl RCloneWrapper {
    ...
    Ok(
      self
        ...
        .filter_map(|mut file_names| {
          if file_names.ends_with("reaper-eth-engine-db-partition-full-range-tables.tar.gz") {
            return None
          }
  ...
  async fn get_all_tarballs(&self) -> eyre::Result<Vec<String>> {
    let result = Command::new("rclone")
      .arg("tree")
      .arg(format!("{}:reaper-eth-engine-db", self.config_name))
      .stdout(Stdio::piped())
      .output()
      .await?;
  ...
  async fn upload_full_range_tables(&self, partition_folder: &PathBuf) -> eyre::Result<()> {
    let directory = PathBuf::from(
      get_dir_content(partition_folder)?
        .directories
        .iter()
        .find(|path| path.ends_with("reaper-eth-engine-db-partition-full-range-tables"))
  ...
  async fn update_block_range_file(&self) -> eyre::Result<()> {
    ...
    let mut file = File::create("/tmp/reaper-eth-engine-available-ranges.json")?;
    let str = serde_json::to_string(&ranges)?;
    write!(&mut file, "{str}")?;

    if !Command::new("rclone")
      .arg("copy")
      .arg("/tmp/reaper-eth-engine-available-ranges.json")
      .arg(format!("{}:reaper-eth-engine-db/", self.config_name))
      .spawn()
      .unwrap()
      .wait()
      ...
      get_dir_content(&partition_folder)?
        ...
        .filter(|path| {
          *path != partition_folder.to_str().unwrap()
            && !path.ends_with("reaper-eth-engine-db-partition-full-range-tables")
        })
