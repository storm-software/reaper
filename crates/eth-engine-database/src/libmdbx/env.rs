...
/// See [reaper_eth_engine_libmdbx::EnvironmentBuilder::set_handle_slow_readers]
...
pub struct DatabaseArguments {
  ...
  /// - Data files MUST be placed in the LOCAL file system, but NOT on a network
  ///   share.
...
impl DatabaseArguments {
  ...
  pub fn new(client_version: ClientVersion) -> Self {
    Self { client_version, log_level: None, max_read_transaction_duration: None, exclusive: None }
  }
  ...
  pub fn open(
         ...
         retry: isize| {
          if space > MAX_SAFE_READER_SPACE {
            let message = if is_current_process(process_id) {
              ...
              "External process has a long-lived database transaction that grows the database \
               file. Use shorter-lived read transactions or shut down the node."
            };
            tracing::warn!(
                target: "reaper-eth-engine::db::mdbx",
                ?process_id,
                ?thread_id,
                ?read_txn_id,
                ?gap,
          ...
          reaper_eth_engine_libmdbx::HandleSlowReadersReturnCode::ProceedWithoutKillingReader
        },
      );
    }
    ...
    if let Some(log_level) = args.log_level {
      ...
      let is_log_level_available = if cfg!(debug_assertions) {
        ...
        matches!(log_level, LogLevel::Fatal | LogLevel::Error | LogLevel::Warn | LogLevel::Notice)
      };
