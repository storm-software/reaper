...
use reaper_eth_engine_metrics::db_reads::LibmdbxMetrics;
...
impl LibmdbxReader for LibmdbxReadWriter {
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"get_dex_quotes")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"load_trace")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"protocol_info")]
  fn get_protocol_details(&self, address: Address) -> eyre::Result<ProtocolInfo> {
    self.db.view_db(|tx| {
        ...
        .protocol_info(true, |handle| handle.get(&address))
      {
        ...
        Some(None) => Err(eyre::eyre!("entry for key {:?} in AddressToProtocolInfo", address)),
        None => tx
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics, scope, db_read,"metadata_no_dex_price")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"metadata")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope, db_read, "try_fetch_token_info")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"try_fetch_searcher_eoa_infos")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"try_fetch_searcher_eoa_info")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"try_fetch_searcher_contract_infos")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"try_fetch_searcher_contract_info")]
  ...
  fn protocols_created_before(
    ...
    self.db.view_db(|tx| {
        ...
        info!(target:"reaper-eth-engine-libmdbx", "loaded {} pairs before block: {}", map.len(), block_num);
  ...
  fn protocols_created_range(
    ...
    self.db.view_db(|tx| {
        ...
        info!(target:"reaper-eth-engine-libmdbx", "loaded {} pairs range: {}..{}", map.len(), start_block, end_block);

        Ok(map)
        })
  }

  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"try_fetch_address_metadatas")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"try_fetch_address_metadata")]
  ...
  #[reaper_eth_engine_macros::metrics_call(ptr=metrics,scope,db_read,"try_fetch_builder_info")]
...
impl DBWriter for LibmdbxReadWriter {
  ...
  async fn write_builder_info(
    ...
    Ok(self.tx.send(
      WriterMessage::BuilderInfo { builder_address, builder_info: Box::new(builder_info) }.stamp(),
    )?)
  }
  ...
  async fn write_block_analysis(
    &self,
    _: reaper_eth_engine_types::db::block_analysis::BlockAnalysis,
  ) -> eyre::Result<()> {
    Ok(())
  }
}
  ...
  fn fetch_block_metadata(&self, block_num: u64) -> eyre::Result<BlockMetadataInner> {
    self.db.view_db(|tx| {
      tx.get::<BlockInfo>(block_num)?.ok_or_else(|| {
        let _ = self.init_state_updating(block_num, META_FLAG, DATA_NOT_PRESENT_NOT_AVAILABLE);
  ...
  pub fn fetch_trades(&self, block: u64) -> eyre::Result<CexTradeMap> {
    self.db.view_db(|tx| {
      tx.get::<CexTrades>(block)?
        ...
        .inspect_err(|_| {
          let _ = self.init_state_updating(block, CEX_TRADES_FLAG, DATA_NOT_PRESENT_NOT_AVAILABLE);
        })
    })
  }
  ...
  pub fn fetch_cex_quotes(&self, block_num: u64) -> eyre::Result<CexPriceMap> {
    self.db.view_db(|tx| {
      let res = tx.get::<CexPrice>(block_num)?.unwrap_or_else(|| {
        let _ =
          self.init_state_updating(block_num, CEX_QUOTES_FLAG, DATA_NOT_PRESENT_NOT_AVAILABLE);
        CexPriceMap::default()
      });
...
pub fn determine_eth_prices(
  ...
  Some(
    cex_quotes
      .get_quote_from_most_liquid_exchange(&Pair(quote_asset, WETH_ADDRESS), block_timestamp, None)?
      .maker_taker_mid()
      .0,
  )
}
