...
use reaper_eth_engine_types::db::{block_times::BlockTimes, cex::CexSymbols};
...
pub struct Clickhouse {
  pub tip:                 bool,
  pub run_id:              u64,
  pub client:              ClickhouseClient<ReaperEthEngineClickhouseTables>,
  pub cex_download_config: CexDownloadConfig,
  pub buffered_insert_tx:  Option<UnboundedSender<Vec<ReaperEthEngineClickhouseData>>>,
}
...
impl Clickhouse {
  pub async fn new(
    ...
    buffered_insert_tx: Option<UnboundedSender<Vec<ReaperEthEngineClickhouseData>>>,
    tip: bool,
    run_id: Option<u64>,
  ) -> Self {
  ...
  pub fn inner(&self) -> &ClickhouseClient<ReaperEthEngineClickhouseTables> {
    &self.client
  }
  ...
  pub async fn get_and_inc_run_id(&self) -> eyre::Result<u64> {
    let id = (self
      .client
      .query_one::<u64, _>("select max(run_id) from reaper.run_id", &())
      .await?
      + 1)
      .into();
  ...
  pub async fn max_traced_block(&self) -> eyre::Result<u64> {
    Ok(
      self
        .client
        .query_one::<u64, _>("select max(block_number) from reaper_api.tx_traces", &())
        .await?,
    )
  }
    ...
    if let Some(tx) = self.buffered_insert_tx.as_ref() {
      ...
      bundle_data.into_iter().try_for_each(|data| {
        match data {
          ...
          BundleData::JitSandwich(s) => tx.send(vec![(s, self.tip, self.run_id).into()])?,
          ...
          BundleData::Liquidation(s) => tx.send(vec![(s, self.tip, self.run_id).into()])?,
  ...
  pub async fn insert_pool(
    ...
    let data = ProtocolInfoClickhouse::new(block, address, tokens, curve_lp_token, classifier_name);
...
impl ClickhouseHandle for Clickhouse {
  ...
  async fn get_metadata(
    ...
    let block_meta =
      BlockMetadataInner::make_new(block_hash, block_timestamp, relay, p2p_timestamp, private_flow);

    let mut cex_quotes_for_block = self
      .get_cex_prices(CexRangeOrArbitrary::Timestamp { block_number: block_num, block_timestamp })
      .await?;

    let cex_quotes = cex_quotes_for_block.remove(0);
    let eth_price =
      determine_eth_prices(&cex_quotes.value, block_meta.block_timestamp * 1_000_000, quote_asset);
    ...
    D: LibmdbxData<T> + DbRow + for<'de> Deserialize<'de> + Send + Debug + 'static,
  {
    ...
    self
      .query_many_with_retry::<D, _>(&query, &())
      .await
      .map_err(Into::into)
  }
  ...
  fn inner(&self) -> &ClickhouseClient<ReaperEthEngineClickhouseTables> {
    &self.client
  }
  ...
  async fn get_cex_prices(
    ...
    let data: Vec<RawCexQuotes> = match range_or_arbitrary {
      ...
      CexRangeOrArbitrary::Timestamp { block_number: _, block_timestamp } => {
        let start_time = (block_timestamp * 1000000) as f64 - (MAX_MARKOUT_TIME * SECONDS_TO_US);
        let end_time = (block_timestamp * 1000000) as f64 + (MAX_MARKOUT_TIME * SECONDS_TO_US);
  ...
  async fn get_cex_trades(
    ...
    let block_times: Vec<BlockTimes> = match range_or_arbitrary {
      CexRangeOrArbitrary::Range(mut s, mut e) => {
        ...
        debug!(
          target = "reaper_eth_engine_db::cex_download",
    ...
    let data: Vec<RawCexTrades> = match range_or_arbitrary {
      CexRangeOrArbitrary::Range(..) => {
        ...
        debug!("Querying raw CEX trades for time range: start={}, end={}", start_time, end_time);
  ...
  pub async fn fetch_symbol_rank(
    ...
    Ok(match range_or_arbitrary {
      CexRangeOrArbitrary::Range(..) => {
        ...
        self
          .query_many_with_retry(MOST_VOLUME_PAIR_EXCHANGE, &(start_time, end_time))
          .await?
      }
      CexRangeOrArbitrary::Arbitrary(_) => {
        ...
        query = query.replace(
          "month >= toStartOfMonth(toDateTime(? / 1000000) - toIntervalMonth(1))) AND (month <= \
           toStartOfMonth(toDateTime(? / 1000000) + toIntervalMonth(1))",
...
mod tests {
  ...
  use reaper_eth_engine_types::{
    ...
    init_thread_pools,
    mev::{
      ArbDetails, AtomicArb, BundleHeader, CexDex, CexDexQuote, JitLiquidity, JitLiquiditySandwich,
      Liquidation, OptimisticTrade, PossibleMev, PossibleMevCollection, Sandwich,
    },
  ...
  async fn test_get_private_flow() {
    ...
    assert_eq!(
      private_flow,
      vec![TxHash::from_str("0x0000000000051fd2c9e99dcb6037d17950a377f21e10ae13f7e3ff0487b74e97")
        .unwrap()]
    );
  }
  ...
  async fn test_get_earliest_p2p_observation() {
    ...
    let block_hash =
      BlockHash::from_str("0xcf384012b91b081230cdf17a3f7dd370d8e67056058af6b272b3d54aa2714fac")
        .unwrap();
    ...
    if relay_meta.is_some() {
      assert_eq!(
        ...
        Some(RelayBlockMetadata {
          ...
          proposer_fee_recipient: Address::from_str("0x992a7a7d9267d114959dd0c9d072d965c4f54419")
            .unwrap(),
  ...
  async fn dex_price_mapping(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    ...
    let query = "SELECT * FROM reaper.dex_price_mapping";
    let queried: DexQuotesWithBlockNumber = db.query_one(query, &()).await.unwrap();

    assert_eq!(queried, case0);
  }

  async fn mev_block(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    let case0_possible = PossibleMev::default();
    let case0 =
      MevBlock { possible_mev: PossibleMevCollection(vec![case0_possible]), ..Default::default() };
  ...
  async fn cex_dex(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
  ...
  async fn cex_dex_quotes(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    let swap = NormalizedSwap {
      ...
      amount_in:   Rational::from_unsigneds(3122757495341445439573u128, 1000000000000000000u128),
  ...
  async fn jit(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    let swap = NormalizedSwap::default();
  ...
  async fn jit_sandwich(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    let swap = NormalizedSwap::default();
    let mint = NormalizedMint::default();
    let burn = NormalizedBurn::default();
    let gas_details = GasDetails::default();
  ...
  async fn liquidations(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    let swap = NormalizedSwap::default();
  ...
  async fn bundle_header(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
  ...
  async fn sandwich(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
  ...
  async fn atomic_arb(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    let swap = NormalizedSwap::default();
    let gas_details = GasDetails::default();
  ...
  async fn pools(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
  ...
  async fn tree(db: &ClickhouseTestClient<ReaperEthEngineClickhouseTables>) {
    ...
    let roots: Vec<_> = tree
      ...
      .iter()
      .map(|root| {
        DbDataWithRunId::<TransactionRoot>::new_with_run_id((root, tree.header.number).into(), 0)
      })
      .collect::<Vec<_>>();
  ...
  async fn test_all_inserts() {
    ...
    let tables = &ReaperEthEngineClickhouseTables::all_tables();
    test_db
