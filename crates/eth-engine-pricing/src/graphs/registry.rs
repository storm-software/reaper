...
/// Manages subgraphs in the ReaperEthEngineBatchPricer module, crucial for DEX
/// pricing.
///
...
/// information.
///
/// Mainly functioning within the ReaperEthEngineBatchPricer system, it plays a
/// key role in providing up-to-date and reliable pricing data in the
/// decentralized exchange context.
#[derive(Debug, Clone)]
  ...
  fn get_price_once(
    ...
    self
      ...
      .or_else(|| {
        Some(
          self
            .get_price_all(unordered_pair, edge_state)
  ...
  pub(crate) fn get_price_all(
    ...
    self.sub_graphs.get(&pair).and_then(|f| {
      ...
      for graph in f.values() {
        ...
        acc += if !unordered_pair.eq_unordered(&default_pair) { next.reciprocal() } else { next };
        cnt += Rational::ONE;
      }
