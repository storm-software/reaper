...
/// Manages the state of pools in the ReaperEthEngineBatchPricer system,
/// maintaining two types of state data: finalized and verification states.
///
...
/// for the ReaperEthEngineBatchPricer system to provide accurate and current
/// pricing information for tokens on decentralized exchanges.
#[derive(Debug, Clone)]
...
impl StateTracker {
  ...
  pub fn all_state_range(&self, block: RangeInclusive<u64>) -> FastHashMap<Address, &PoolState> {
    self
      .state_for_verification_range(block)
      .into_iter()
      .chain(self.finalized_state())
      .collect()
  }
