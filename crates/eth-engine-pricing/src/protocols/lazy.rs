...
impl Drop for MultiBlockPoolFutures {
  fn drop(&mut self) {
    ...
    tracing::debug!(target: "reaper-eth-engine::mem", rem_futures=futures_cnt, "current state fetch futures in pricing");
  }
}
...
impl Stream for MultiBlockPoolFutures {
  ...
  fn poll_next(
    ...
    let (mut results, empty): (Vec<_>, Vec<_>) = self
      ...
      .map(|(block, futures)| {
        let res = if let Poll::Ready(result) = futures.poll_next_unpin(cx) { result } else { None };
