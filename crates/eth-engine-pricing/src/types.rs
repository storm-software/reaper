...
impl PoolUpdate {
  ...
  pub fn get_pair(&self, quote: Address) -> Option<Pair> {
    match &self.action {
      ...
      Action::Mint(m) => {
        Some(Pair(m.token.first()?.address, m.token.get(1).map(|t| t.address).unwrap_or(quote)))
      }
      Action::Burn(b) => {
        Some(Pair(b.token.first()?.address, b.token.get(1).map(|t| t.address).unwrap_or(quote)))
      }
      Action::Collect(b) => {
        Some(Pair(b.token.first()?.address, b.token.get(1).map(|t| t.address).unwrap_or(quote)))
      }
