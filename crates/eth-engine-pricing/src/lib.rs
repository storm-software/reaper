...
use futures::Stream;
pub use graphs::{
  AllPairGraph, GraphManager, StateTracker, SubGraphRegistry, SubgraphVerifier, VerificationResults,
};
...
impl<T: TracingProvider> ReaperEthEngineBatchPricer<T> {
  ...
  fn on_pool_updates(&mut self, updates: Vec<PoolUpdate>) {
    ...
    updates.iter().for_each(|msg| {
      ...
      self
        .graph_manager
        .prune_low_liq_subgraphs(pair0, self.quote_asset, self.completed_block + 1);
      self
        .graph_manager
        .prune_low_liq_subgraphs(pair1, self.quote_asset, self.completed_block + 1);
    });
    ...
    pools
      .into_iter()
      .flatten()
      .for_each(|NewGraphDetails { pair, extends_pair, block, edges }| {
        if edges.is_empty() {
  ...
  fn par_rundown(&mut self, pairs: Vec<(PairWithFirstPoolHop, u64)>) {
    ...
    let new_subgraphs = execute_on!(target = pricing, {
      pairs
        ...
        .map(|queries| {
          ...
          let (edges, mut extend): (Vec<_>, Vec<_>) = par_state_query(&self.graph_manager, queries)
            .into_iter()
    ...
    let verify = new_subgraphs
      ...
      .filter_map(|(pair, extend, block, edges, frayed_ext)| {
        let (id, need_state, ..) = self.add_subgraph(pair, extend, block, edges, frayed_ext)?;
  ...
  fn handle_drastic_price_changes(&mut self, prices: &mut DexQuotes) {
    ...
    removals.into_iter().for_each(|pair| {
      tracing::debug!(target: "reaper-eth-engine::missing_pricing",pair=?pair.0,
