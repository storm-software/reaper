...
use reaper_eth_engine_database::clickhouse::Clickhouse;
#[cfg(not(feature = "local-clickhouse"))]
use reaper_eth_engine_database::clickhouse::ClickhouseHttpClient;
pub use reaper_eth_engine_database::libmdbx::{DBWriter, LibmdbxReadWriter, LibmdbxReader};
use reaper_eth_engine_database::{
  libmdbx::LibmdbxInit, AddressToProtocolInfo, PoolCreationBlocks, Tables, TokenDecimals,
};
use reaper_eth_engine_metrics::ParserMetricEvents;
...
impl TraceLoader {
  ...
  pub async fn get_metadata(
    ...
    if pricing {
      if let Ok(res) = self.test_metadata_with_pricing(block, USDT_ADDRESS) {
        ...
        self
          .test_metadata_with_pricing(block, USDT_ADDRESS)
  ...
  pub async fn fetch_missing_traces(&self, block: u64) -> eyre::Result<()> {
    ...
    let tables =
      Arc::new(vec![(Tables::TxTraces, Tables::TxTraces.build_init_state_progress_bar(&multi, 4))]);
  ...
  pub async fn get_tx_traces_with_header(
    ...
    for res in join_all(tx_hashes.into_iter().map(|tx_hash| async move {
      ...
      Ok::<_, TraceLoaderError>(TxTracesWithHeaderAnd { block, tx_hash, trace, header, other: () })
    }))
    .await
    {
      let res = res?;
  ...
  *DB_HANDLE
    .get_or_init(|| async {
      ...
      let this = &*Box::leak(Box::new(
        LibmdbxReadWriter::init_db_tests(&reaper_eth_engine_db_path).unwrap_or_else(|e| {
          panic!("failed to open db path {}, err={}", reaper_eth_engine_db_path, e)
        }),
      ));
      ...
      if init_crit_tables(this) {
        ...
        this
          .initialize_full_range_tables(clickhouse, tracer.get_tracer(), false)
          .await
          .unwrap();
      } else {
...
fn init_crit_tables(db: &LibmdbxReadWriter) -> bool {
  ...
  let tables = &[Tables::PoolCreationBlocks, Tables::AddressToProtocolInfo, Tables::TokenDecimals];
  ...
  for table in tables {
    let cnt = match table {
      ...
      Tables::AddressToProtocolInfo => db.get_table_entry_count::<AddressToProtocolInfo>().unwrap(),
