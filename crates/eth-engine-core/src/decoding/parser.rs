...
use reaper_eth_engine_metrics::trace::types::{BlockStats, TraceParseErrorKind, TransactionStats};
#[cfg(feature = "dyn-decode")]
...
impl<T: TracingProvider, DB: LibmdbxReader + DBWriter> TraceParser<T, DB> {
  ...
  pub(crate) async fn fill_metadata(
    ...
    let (traces, tx_stats): (Vec<_>, Vec<_>) = join_all(
      block_trace
        .into_iter()
  ...
  async fn parse_transaction(
    ...
    let stats =
      TransactionStats { block_num, tx_hash, tx_idx: tx_idx as u16, traces: vec![], err: None };
