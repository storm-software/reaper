...
use reaper_eth_engine_types::structured_trace::{TransactionTraceWithLogs, TxTrace};
use reth_primitives::{Bytes, U64};
use reth_rpc_types::{trace::parity::*, TransactionInfo};
use revm::{
  inspectors::GasInspector,
  interpreter::{
    opcode, CallInputs, CallOutcome, CallScheme, CreateInputs, CreateOutcome, InstructionResult,
...
///
/// The [ReaperTracingInspector] keeps track of everything by:
...
#[derive(Clone, Debug)]
pub struct ReaperTracingInspector {
...
impl ReaperTracingInspector {
  ...
  pub fn build_trace(
    ...
    let mut traces: Vec<TransactionTraceWithLogs> = Vec::with_capacity(self.traces.nodes().len());
  ...
  pub(crate) fn parity_selfdestruct_trace(
    ...
    Some(TransactionTrace { action: trace, error: None, result: None, trace_address, subtraces: 0 })
  }
...
impl<DB> Inspector<DB> for ReaperTracingInspector
where
  DB: Database,
{
  #[inline]
