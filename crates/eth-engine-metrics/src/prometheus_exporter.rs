...
pub(crate) async fn initialize_with_hooks<F: Hook + 'static>(
  ...
  // Build metrics stack
  Stack::new(recorder)
    .push(PrefixLayer::new("reaper-eth-engine"))
    .install()
...
fn collect_memory_stats() {
  ...
  if let Ok(value) =
    stats::active::read().map_err(|error| error!(%error, "Failed to read jemalloc.stats.active"))
  {
  ...
  if let Ok(value) =
    stats::mapped::read().map_err(|error| error!(%error, "Failed to read jemalloc.stats.mapped"))
  {
...
fn collect_io_stats() {
  ...
  let Ok(io) = process
    .io()
    .map_err(|error| error!(%error, "Failed to get IO stats for the currently running process"))
  else {
    return
  };
