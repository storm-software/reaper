        ...
        (
          ...
          *ignore_before,
          vec![log_ident]
            .into_iter()
            .chain(log_fallbacks)
            .map(|log_ident| Ident::new(&(log_ident.to_string() + "_field"), Span::call_site()))
            .collect::<Vec<_>>(),
          vec![log_ident.clone()]
            .into_iter()
      ...
      .flat_map(|(names, repeating)| {
        names
          ...
          .map(|name| {
            ...
            let data_type =
              if *repeating { quote!(Vec<#mod_path::#name>) } else { quote!(#mod_path::#name) };
      ...
      .flat_map(|names| {
        names
          ...
          .map(|name| {
            let field = Ident::new(&(name.to_string() + "_field"), Span::call_site());
            let message = format!(
              "logs are not setup properly for this macro as the requested log {} was not found",
              name
            );
