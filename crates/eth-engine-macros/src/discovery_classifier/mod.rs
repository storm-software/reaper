...
pub fn discovery_impl(token_stream: TokenStream) -> syn::Result<TokenStream> {
  ...
  Ok(quote! (
      ...
      mod #mod_name {
          ...
          use super::*;
          use ::reaper_eth_engine_types::normalized_actions::pool::NormalizedNewPool;
          ...
          impl crate::FactoryDiscovery for #discovery_name {
              ...
              async fn decode_create_trace<T: ::reaper_eth_engine_types::traits::TracingProvider>(
                  &self,
                  tracer: ::std::sync::Arc<T>,
                  deployed_address: ::alloy_primitives::Address,
                  trace_idx: u64,
                  parent_calldata: ::alloy_primitives::Bytes,
              ) -> Vec<::reaper_eth_engine_types::normalized_actions::pool::NormalizedNewPool>{
                  let Ok(decoded_data) = <#function_call_path
                      as ::alloy_sol_types::SolCall>::abi_decode(&parent_calldata[..], false)
                      else {
                          ::tracing::error!(target: "reaper_eth_engine_classifier::discovery", "{} failed to decode calldata", #discovery_name_str);
                          return Vec::new();
                  };
                  let res = (#address_call_function)
                      ...
                      .await;
                  if res.is_empty() {
                          ::tracing::error!(target: "reaper_eth_engine_classifier::discovery", "discovery classifier returned nothing");
                  }
...
pub fn discovery_dispatch(input: TokenStream) -> syn::Result<TokenStream> {
  ...
  Ok(quote!(
      ...
      impl crate::FactoryDiscoveryDispatch for #struct_name {
          async fn dispatch<T: ::reaper_eth_engine_types::traits::TracingProvider>(
                  &self,
                  tracer: ::std::sync::Arc<T>,
                  ...
                  ::futures::stream::iter(search_data)
                      ...
                      if parent_calldata.len() < 4 {
                          ::tracing::debug!(target: "reaper_eth_engine_classifier::discovery", ?deployed_address, ?factory, "invalid calldata length");
                          return Vec::new()
                      }
                      ...
                      match key {
                          #(
                              #var_name => {
                              ::tracing::trace!(target: "reaper_eth_engine_classifier::discovery", ?deployed_address, ?factory, ?key, "match found");
                              return
                          ...
                          _ => {
                              ::tracing::trace!(target: "reaper_eth_engine_classifier::discovery", ?deployed_address, ?factory, ?key, "no match found");
                              Vec::new()
                          }
                      }
                  }
...
impl Parse for DiscoveryDispatch {
  fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
    ...
    if !input.is_empty() {
      return Err(syn::Error::new(Span::call_site(), "no discovery implementations to dispatch to"))
    }
