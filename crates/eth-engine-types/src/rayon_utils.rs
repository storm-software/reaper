...
macro_rules! execute_on {
    ...
    (pricing, $block:block) => {
        ::reaper_eth_engine_types::execute_on_pricing_thread_pool(|| $block)
    };
    (inspect, $block:block) => {
        ::reaper_eth_engine_types::execute_on_inspect_thread_pool(|| $block)
    };
    (async_inspect, $block:block) => {
        ::reaper_eth_engine_types::execute_on_inspect_thread_pool_async(move || $block)
    };
}
