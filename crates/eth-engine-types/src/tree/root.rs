...
impl GasDetails {
  ...
  pub fn merge(&mut self, other: &GasDetails) {
    self.coinbase_transfer = Some(
      self.coinbase_transfer.unwrap_or_default() + other.coinbase_transfer.unwrap_or_default(),
    )
    .filter(|&res| res != 0);
  ...
  pub fn pretty_print_with_spaces(&self, f: &mut fmt::Formatter, spaces: usize) -> fmt::Result {
    ...
    for (label, value) in &labels {
      writeln!(f, "{}", self.format_line_with_spaces(label, value, max_label_length, &space_str))?;
    }
...
pub mod test {
    ...
    use reaper_eth_engine_classifier::test_utils::{get_db_handle, ClassifierTestUtils};
    use reaper_eth_engine_types::{normalized_actions::Action, tree::BlockTree};
