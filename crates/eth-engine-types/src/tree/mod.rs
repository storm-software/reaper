...
impl<V: NormalizedAction> BlockTree<V> {
  ...
  pub fn get_tx_info_batch<DB: LibmdbxReader>(
    ...
    roots
      ...
      .map(|root| {
        root
          .get_tx_info_batch(self.header.number, &eoa, &contract, &address_meta)
          .ok()
      })
      .collect()
  }
  ...
  pub fn get_tx_info<DB: LibmdbxReader>(&self, tx_hash: B256, database: &DB) -> Option<TxInfo> {
    self
      ...
      .and_then(|root| {
        root
          .get_tx_info(self.header.number, database)
...
pub mod test {
  ...
  use reaper_eth_engine_types::{normalized_actions::Action, BlockTree, TreeSearchBuilder};
