...
impl<'a> PairTradeWalker<'a> {
  ...
  pub(crate) fn get_trades_for_window(&mut self) -> Vec<CexTradePtr<'a>> {
    ...
    for (exchange, trades) in &self.trades {
      let Some((lower_idx, upper_idx)) = self.exchange_ptrs.get_mut(exchange) else { continue };
...
impl<'a> SortedTrades<'a> {
  pub fn new_from_cex_trade_map(
    ...
    let mut consolidated_trades: FastHashMap<&'a Pair, Vec<&'a CexTrades>> = FastHashMap::default();
...
pub fn log_missing_trade_data(dex_swap: &NormalizedSwap, tx_hash: &TxHash) {
  trace!(
      target: "reaper::time_window_vwam::missing_trade_data",
...
pub fn log_insufficient_trade_volume(
  ...
  trace!(
      target: "reaper::time_window_vwam::insufficient_trade_volume",
