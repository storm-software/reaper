...
impl<'a> SortedTrades<'a> {
  ...
  pub(crate) fn get_optimistic_price(
    ...
    if res.is_none() {
      tracing::debug!(target: "reaper_eth_engine_types::db::cex::optimistic", ?pair, "No price VMAP found for {}-{} in optimistic time window. \n Tx: {}", dex_swap.token_in.symbol, dex_swap.token_out.symbol, format_etherscan_url(&tx_hash));
    }
  ...
  fn get_optimistic_via_intermediary(
    ...
    self.calculate_intermediary_addresses(&pair)
            ...
            .filter_map(|intermediary| {
                ...
                tracing::debug!(target: "reaper_eth_engine_types::db::cex::trades::optimistic", ?pair, ?intermediary, "trying via intermediary");
  ...
  fn get_optimistic_direct(
    ...
    let mut baskets_queue = TimeBasketQueue::new(trade_data, block_timestamp, quality_pct, &config);

    baskets_queue.construct_time_baskets();

    while baskets_queue.volume.lt(volume) {
      if baskets_queue.get_min_time_delta(block_timestamp) >= config.max_optimistic_pre_block_us
        || baskets_queue.get_max_time_delta(block_timestamp) >= config.max_optimistic_post_block_us
      {
        break
      }
