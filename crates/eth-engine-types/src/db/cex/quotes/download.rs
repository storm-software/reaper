...
impl CexQuotesConverter {
  ...
  pub fn create_price_map(
    ...
    exchange_map
      ...
      .map(|(exch, quote_indices)| {
        let mut exchange_pair_index_map: FastHashMap<Pair, Vec<usize>> = FastHashMap::default();
  ...
  pub fn create_block_num_map_with_pairs(
    ...
    for (index, quote) in self.quotes.iter().enumerate() {
      let (matching_blocks, latest_block) = self.find_matching_blocks(quote.timestamp, last_block);
